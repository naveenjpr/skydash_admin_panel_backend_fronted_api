  ...
  return (
    <>
      {filterData.map((v) => {
        return (
          <div
            className={` duration-500 border hover:shadow-[0px_0px_15px_1px_gray] border-gray-300  w-full mb-5 group rounded-[10px_10px_0px_0px] overflow-hidden`}
          >
            <div
              className="cardimg   h-[200px] bg-cover bg-no-repeat relative"
              style={{ backgroundImage: `url('${v.image}')` }}
            >
              <div className="bg-[rgba(0,0,0,0.5)] w-full h-full absolute top-0 left-0">
                <button className="bg-yellow-600 absolute duration-[0.3s] scale-0 group-hover:scale-[1] top-3 left-3 text-white px-3 py-2 text-[15px] rounded-md">
                  Top Seller
                </button>
                <span className="absolute scale-0 origin-center duration-[0.3s] group-hover:scale-[1s] group-hover:bottom-[80px] text-white bottom-[30px] left-[105px]">
                  Preview Course
                </span>
              </div>
            </div>
